...
executable targeting the x86-64 \textit{isa} and the Linux system call
...
(\(\mathrm{IPC}/\mathrm{IPC}_{\text{baseline}}\)), while numeric sweeps (e.g.,
cache line size) are plotted as absolute values, with logarithmic scales
applied where appropriate to span orders of magnitude.

\subsection{Datasets}\label{sub:dataset-size}

PolyBench kernels are parameterized by dataset size through preprocessor
definitions (e.g., \texttt{SMALL\_DATASET}, \texttt{MEDIUM\_DATASET},
\texttt{LARGE\_DATASET}), with \texttt{LARGE\_DATASET} selected by default if
none is specified. In this work, specifically, dataset sizes were explicitly
configured per kernel in order to balance simulation fidelity with tractable
execution time under detailed timing simulation. Table~\ref{tab:datasets}
summarizes the selected dataset for each workload along with the corresponding
problem dimensions.
...
\texttt{pyenv} and a project virtual environment; gem5 submodule initialization
...
\texttt{pyenv}, and all Python packages are frozen with \texttt{pip~freeze}.
...
binaries against musl to reduce host-library variance. The only component not
strictly pinned is the C toolchain; we run simulations with GCC version~\(\geq
14\) and observe no material effect on the comparative cache trends within that
range. All invocations of gem5, simulations, and analysis are routed through
the project \texttt{venv} to ensure a consistent runtime across machines.
...
build graph and the runner provide a concise reproduction path.
