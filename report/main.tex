...
\begin{abstract}
	...
	\textsc{mpki} decomposition reveal where increased capacity or
	associativity yields diminishing returns. The artifacts and methodology aim
	to help students and researchers perform rigorous, replicable cache
...
kernels using gem5. We establish a three-level baseline cache configuration and
simulate kernel executions while sweeping over orthogonal parameters one at a
time. The first of these parameters is the cache hierarchy size, which is split
into 11 presets (see Table~\ref{tbl:cachesizes}) roughly inspired by real
commercial \textsc{cpu}s,\footnote{For the Intel Atom and IBM POWER10
configurations, cache sizes were adjusted to the nearest power-of-two values to
ensure compatibility with the gem5 simulator. Apple Silicon cache
sizes are taken from the energy-efficiency cores.} where the Intel® Core™
i7-6700K was chosen as the baseline. Next, we sweep the size of the cache lines
over four power-of-two values, from 32 to 256 bytes, where the baseline is 64
bytes; and finally we sweep  L1, L2, and L3 associativities individually over
1-way (directly mapped), 2-way, 4-way, 8-way, and 16-way
configurations,\footnote{A 12-way associativity configuration would be
desirable, as it exists in the Zen~5 architecture, but gem5 expects
power-of-two associativities.} where the baselines were 8-way for L1 and L2,
and 16-way for L3.
...
because the effects of out-of-order execution on cache metrics is not in the
scope of this experiment. The system operates at a clock frequency of 4~GHz and
models a timing-based memory system over a physical address space of 8~GiB. The
processor is connected to a multi-level cache hierarchy consisting of private
L1 instruction and data caches, followed by shared L2 and L3 caches. Each cache
level is equipped with a finite number of Miss Status Holding Registers
(\textsc{mshr}s), to enable multiple outstanding memory requests and allow
limited memory-level parallelism. Communication between cache levels and main
memory is modeled using a hierarchical crossbar interconnect, with dedicated
crossbars between each level and a data width of 32 bytes. See
Fig.~\ref{fig:system-diagram} for the system diagram generated by the
simulator.
...
We use five PolyBench Kernels \cite{polybench:2016}
...
\section{Methodology \textit{bad}}
\subsection{Testing Strategy with gem5}
\subsubsection{General Approach Description}

Tests were conducted using the gem5 simulator, enabling precise simulation of
cache behavior across different hardware configurations. All workloads were
compiled for x86 instruction set architecture, ensuring compatibility with the
gem5 x86 simulator backend. For each workload, 31 distinct simulations were
performed with varying cache configurations, enabling extensive analysis of
cache parameters' impact on performance.

\subsubsection{Automation Framework (run\_all\_simulations.py)}

The \texttt{run\_all\_simulations.py} script serves as the central orchestrator
for simulations, implementing the following features:

\begin{itemize}
	\item \textbf{Parallel Execution}: Uses \texttt{multiprocessing} to
		distribute simulations across multiple processors, significantly
		reducing total execution time
	\item \textbf{Core Affinity}: Implements \textsc{cpu} pinning to improve
		data locality and reduce measurement variability
	\item \textbf{Output Synchronization}: Ensures logs from different
		processes do not mix during parallel execution
	\item \textbf{Completion Verification}: Automatically detects successful
		simulations by the presence of \texttt{stats.txt} with critical metrics
		(\texttt{simTicks}, \texttt{system.cpu.numCycles})
\end{itemize}

\subsubsection{Tested Cache Configurations}

Each workload was tested against 11 realistic cache configurations based on
real commercial architectures:\footnote{For the Intel Atom and IBM POWER10
configurations, cache sizes were adjusted to the nearest power-of-two values to
ensure compatibility with the gem5 simulator.}

\begin{table}[h]
	\centering
	\caption{Tested cache configuration presets}
	\begin{tabular}{|l|c|c|c|c|c|}
		\hline
		\textbf{Preset}              & \textbf{L1I} & \textbf{L1D} & \textbf{L2} & \textbf{L3} \\
		\hline
		\hline
		i7-6700K \textit{(baseline)} & 32 KiB       & 32 KiB       & 256 KiB     & 8 MiB       \\
		Small Embedded               & 16 KiB       & 16 KiB       & 128 KiB     & 1 MiB       \\
...
All cache dimensions and associativity values were rounded to powers of 2, as
required by the gem5 simulator.

Totaling 31 different variations per workload (11 base configurations + cache
line variations + associativity variations).
...
\paragraph{Type}: Linear Algebra | \paragraph{Access Pattern}: Sequential with
Tiling

\paragraph{Description}

\textsc{blas}-level implementation of generalized matrix multiplication: $C :=
\alpha \cdot A \cdot B + \beta \cdot C$. Uses parameterized datasets from the
PolyBench benchmark.
...
\begin{itemize}
	...
	\item \textbf{Cache Hit Rate}: Expected high, especially with cache
		blocking
	\item \textbf{Cache Sensitivity}: High - optimized configurations can
		double performance
\end{itemize}
...
\begin{itemize}
	\item \textbf{Temporal}: $O(NI \times NJ \times NK) = O(200 \times 220
		\times 240) = O(10.56 \times 10^{6})$ operations
	\item \textbf{Spatial}: $O(NI \times NK + NK \times NJ + NI \times NJ)
		\approx 1.1$ MB with double precision
\end{itemize}
...
Computes matrix transpose application followed by vector multiplication: $y :=
A^T \cdot (A \cdot x) + z$. Demonstrates non-sequential access patterns in one
dimension.
...
\begin{itemize}
	\item \textbf{Temporal Locality}: Moderate (y and z redefinition in inner
		loops)
	\item \textbf{Spatial Locality}: Moderate-High (column access in second
		stage)
...
Classic dynamic programming algorithm for finding shortest paths between all
pairs of vertices. Demonstrates complex data reuse pattern with three levels of
nesting.
...
\begin{itemize}
	\item \textbf{Temporal Locality}: Very High (path[i][j], path[i][k],
		path[k][j] reused)
...
\begin{itemize}
	\item \textbf{Temporal}: $O(N^3) = O(180^3) \approx 5.8 \times 10^{6}$
		comparisons
	\item \textbf{Spatial}: $O(N^2) \approx 129.6$ \textsc{kb} for distance
		matrix (integers)
\end{itemize}
...
Implementation of the Jacobi iterative method for solving linear systems in 2D.
Uses ``stencil'' access pattern where each point depends on its 5 neighbors
...
\begin{itemize}
	\item \textbf{Temporal Locality}: Moderate-High (alternation between A and
		B)
	\item \textbf{Spatial Locality}: Very High (access to 5 neighbors close
		together)
...
Heat transfer simulation, partial differential equation solving, computational
fluid dynamics.
...
Variant of the Jacobi method using Gauss-Seidel sweep, where each point uses
already-updated values in the current iteration. Presents slight reduction in
locality compared to Jacobi.
...
\begin{itemize}
	\item \textbf{Temporal Locality}: Moderate (limited reuse between
		timesteps)
	\item \textbf{Spatial Locality}: Very High (9 neighbors accessed
		sequentially)
	\item \textbf{Cache Hit Rate}: Very High - better than Jacobi in some
		aspects
...
Uses 9 points in Moore neighborhood versus 5 points in von Neumann
neighborhood, resulting in larger data volume accessed but possibly better
convergence.

\subsection{Justification for Workload Selection}

\begin{enumerate}
	\item \textbf{PolyBench Suite}: Widely accepted benchmark set in the
		academic community for scientific computing
	\item \textbf{Diversity}: Covers access patterns from sequential to complex
		global patterns
	\item \textbf{Realism}: Reflect practical applications (simulations, linear
		algebra, graph processing)
	\item \textbf{Complete Coverage}: Encompass computational stencil, linear
		algebra, and graph algorithms
\end{enumerate}
...
highly sequential access patterns (\textsc{ipc} improves accordingly), probably
because larger lines increase spatial locality utilization per miss, and
associativity effects are mostly tied to cache entry conflicts within a set. As
such, associativity effects were largely flat, and the capacity presets impact
...
\textsc{gemm}, arguably due to inherent temporal locality and compute
intensity.
...
different workloads were set to use different dataset size configurations due
to time constraints. The \textsc{ata}x kernel was compiled with the
LARGE\_DATASET definition, the Floyd-Warshall kernel with SMALL\_DATASET, and
the remaining kernels with MEDIUM\_DATASET. This may explain why the
\textsc{ata}x simulations exhibited the largest absolute jumps in
\textsc{mpki}, miss rates, total latencies, and \textsc{cpi}, whereas
Floyd-Warshall exhibited the smallest jumps: larger dataset sizes may increase
working-set pressure, such that, if the working set is near the cache size
boundary, changes in total cache capacity and line size may materially change
miss behavior.
